...
但很快会发现，其中最麻烦的其实是第一个词：**区分力本身并不是一个单一概念。**

一条 observation 可以排除更多 possible worlds，却未必更有利于当前行动；一个 channel 也可能不是简单地比另一个“强”或“弱”；多条 evidence 同时出现时，它们还可能重复、互补，甚至只有组合以后才产生新的区分。

所以接下来真正要拆开的，是三个相互关联的问题：

> **它区分了多少？它对当前行动有多大价值？为了得到它，又值得付出多少成本？**

其中，“区分了多少”还会继续分成几层：减少多少 uncertainty、一个 channel 是否包含另一个的能力、多个 channels 放在一起以后到底增加了什么。
...
> **决策价值可以告诉我们当前应该选谁，却不能自动回答一条 information source 在更一般意义上有多强的区分力。**
...
于是接下来先把“决策价值”放到一边，回头认真拆第一个词：区分力。
...
两个答案在比较不同的东西：一个在比较区分力，一个在比较决策价值。
...
> **一项 observation 平均让我们少了多少 uncertainty？**

但区分力似乎还不止这一层。

如果两个 information sources 看的是同一件事，其中一个明显只是另一个的“劣化版”，我们还想表达一种比“少了多少 bits”更结构化的强弱关系。
...
> **不固定一个具体任务，而问一个 channel 是否完整保留了另一个 channel 能够提供的决策能力。**

可是现实中的系统通常不只有一个 channel。

两个 channels 单独各自有区分力，不代表放在一起以后就可以把两边的分数直接相加。
...
对这篇文章来说，它最重要的作用只是继续拆“区分力”这个词：

> **多个 channels 的总区分力，不能只看每一个 channel 单独有多少 information，还要看它们彼此是什么关系。**
...
到这里，我们花了三章，其实一直只是在拆开头的第一个词：**区分力**。
...
第四篇之所以没有在 Chapter 1 就结束，是因为我们还想回答另一个层次的问题：当任务未知、任务会变化，或者我们只是想比较 information sources 本身时，“区分力”还能有哪些不同含义？
...
- 多个 channels 之间是重复、独有还是协同，它们的错误又是否相关。
...
真正清楚以后，我们得到的不是一个万能的信息分数，而是一组彼此相关但不能互相替代的判断。

> **information 不是事实数量的简单累加，而是 observation、hidden state、已有 evidence、行动目标和获取条件之间的关系。**
